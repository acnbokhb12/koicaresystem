...
function calculateTotalMoney(totalElements) {
    let totalAmount = 0;
...
// Sử dụng ví dụ
const totalElements = document.querySelectorAll('.total-item'); // Chọn tất cả các phần tử cần tính
const total = calculateTotalMoney(totalElements);
...
function renderCartItems(cart) {
    const htmlItems = `
        ...
        <section  class="cart__about__property">
                ${cart.map(function (c) {
        return `
                        <div  class="cart__about__property-desc row" data-idproductwasremove=${c.product.productId}>
                <!-- img -->
                    ...
                    <div class="row cart__about__property-infor-row ">
                        ...
                        <div class="cart__about__property-infor-desc col-md-6">
                            ...
                            <div class="cart__about__property-quantity ">
                                <h3>Quantity: <span>${c.quantity}</span></h3>
                            </div>
                            <div class="cart__about__property-quantity ">
                        ...
                        <div class="cart__about__property-price col-md-3">
                            <h3 class="property-price-cart">${c.unitPrice}</h3>
                        </div>
                        <!-- total -->
                        <div class="cart__about__property-total col-md-3">
                            <h3 class="property-total-cart">${c.totalPrice}</h3>
                        </div>
                    </div>
                </div>
            </div> `
    ...
    `;
    let totalAmount = 0;
    let quantityALLCateProduct = 0
    document.getElementById('contain__cart-items').innerHTML = htmlItems;
    ...
    quantityALLCateProduct = priceElements.length;
    ...
    btnDeletes.forEach(function (tick, index) {
        tick.addEventListener('click', function (e) {
            ...
            createDeleteModal(name, async () => {
                await removeItemFromCard(proID,index);
            });
        });
    });
...
async function removeItemFromCard(productID,index) {
    try {
        ...
        if (listCartItem && listCartItem.length > 0) { 
            ...
            document.querySelector(`div[data-idproductwasremove="${productID}"]`).remove();
            document.querySelector('.cart__title-description').remove();
            document.querySelector('.cart__about__check-subtotal').remove();
...
function removeAtIndex( index) {
    if (index >= 0 && index < listCartItem.length) { // Kiểm tra index hợp lệ
        listCartItem.splice(index, 1); // Xóa phần tử tại index
    } else {
        console.error("Index không hợp lệ!");
    }
}

function rendetTotalCart(totalMoney, quantityALLCateProduct) {
    const htmlTotal = `
         <div class="cart__about__check-subtotal-container">
                <div class="row cart__about__check-subtotal-container-row">
                    <div class="check-subtotal-quantity-total-item col-md-4">
                        <h3>Subtotal ( <span>${quantityALLCateProduct} Items</span> ):</h3>
                    </div>
                    <div class="check-subtotal-quantity-price-item col-md-4">
                        <h3 id="subtotal-quantity-price-item">${totalMoney} </h3>
                    </div>
                ...
                <div class="row">
                    <div class="col-12 desc-cart-empty">
                        ...
                        <img src="https://blogzine.webestica.com/assets/images/icon/empty-cart.svg"
                            class="h-200 h-md-300 mb-3" alt="">
                        <!-- Subtitle -->
                        <h2>Your cart is currently empty</h2>
                        <!-- info -->
                        <p class="mb-0">I think the below button is important. Hit this button and you will find a lot
